  ...
  <?php if ($rows): ?>
    <div class="view-content">
		...
		foreach($pdata as $key=>$val){
			// list($nameKey, $versionKey) = explode('_', $val['field_programming_languages']);
			$part = explode('_', $val['field_programming_languages']);
			$nameKey = @$part[0];
			if(isset($nameKey)){	
				if (isset($piearr[$nameKey]) && !empty($piearr[$nameKey])) {
					//$piearr[$nameKey] += $val['field_programming_languages_1'];
				}else{
					$piearr[$nameKey] = $val['field_programming_languages_1'];
				}
			}
		...
		?>
		<div id="programming_languages" class="gov-wide-chart" style="">&nbsp;</div>
		<script type="text/javascript">
		Highcharts.chart('programming_languages', 
		{
