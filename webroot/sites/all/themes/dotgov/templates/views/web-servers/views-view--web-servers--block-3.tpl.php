  ...
  <?php if ($rows): ?>
    <div class="view-content">
		...
		foreach($pdata as $key=>$val){
			// list($nameKey, $versionKey) = explode('_', $val['field_cms_applications']);
			$part = explode('_', $val['field_cms_applications']);
			$nameKey = @$part[0];
			if(isset($nameKey) && !empty($nameKey)){	
				if (isset($piearr[$nameKey]) && !empty($piearr[$nameKey])) {
					//$piearr[$nameKey] += $val['field_cms_applications_1'];
				}else{
					$piearr[$nameKey] = $val['field_cms_applications_1'];
				}
			}
		...
		?>
		<div id="cms_applications" class="gov-wide-chart" style="">&nbsp;</div>
		<script type="text/javascript">
		Highcharts.chart('cms_applications', 
		{
