  ...
  <?php if ($rows): ?>
    <div class="view-content">
		...
		foreach($pdata as $key=>$val){
			// list($nameKey, $versionKey) = explode('_', $val['field_operating_systems']);
			$part = explode('_', $val['field_operating_systems']);
			$nameKey = @$part[0];
			if(isset($nameKey) && !empty($nameKey)){	
				if (isset($piearr[$nameKey]) && !empty($piearr[$nameKey])) {
					//$piearr[$nameKey] += $val['field_operating_systems_1'];
				}else{
					$piearr[$nameKey] = $val['field_operating_systems_1'];
				}
			}
		...
		?>
		<div id="operating_systems" class="gov-wide-chart" style="">&nbsp;</div>
