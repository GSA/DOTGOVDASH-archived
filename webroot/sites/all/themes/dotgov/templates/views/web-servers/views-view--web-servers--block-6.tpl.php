  ...
  <?php if ($rows): ?>
    <div class="view-content">
		...
		foreach($pdata as $key=>$val){
			// list($nameKey, $versionKey) = explode('_', $val['field_analytics_applications']);
			$part = explode('_', $val['field_analytics_applications']);
			$nameKey = @$part[0];
			if(isset($nameKey) && !empty($nameKey)){	
				if (isset($piearr[$nameKey]) && !empty($piearr[$nameKey])) {
					//$piearr[$nameKey] += $val['field_analytics_applications_1'];
				}else{
					print_r($piearr[$nameKey]);
					$piearr[$nameKey] = $val['field_analytics_applications_1'];
				}
			}
		}
		?>
		<div id="analytics_applications" class="gov-wide-chart" style="">&nbsp;</div>
		<script type="text/javascript">
		Highcharts.chart('analytics_applications', 
		{
