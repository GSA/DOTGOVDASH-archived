  ...
  <?php if ($rows): ?>
    <div class="view-content">
		...
		foreach($pdata as $key=>$val){
			// list($nameKey, $versionKey) = explode('_', $val['field_web_frameworks']);
			$part = explode('_', $val['field_web_frameworks']);
			$nameKey = @$part[0];
			if(isset($nameKey) && !empty($nameKey)){
				if (isset($piearr[$nameKey]) && !empty($piearr[$nameKey])) {
					// $piearr[$nameKey] += $val['field_web_frameworks_1'];
				}else{
					$piearr[$nameKey] = $val['field_web_frameworks_1'];
				}
			}
		...
		?>
		<div id="web_frameworks" class="gov-wide-chart" style="">&nbsp;</div>
		<script type="text/javascript">
		Highcharts.chart('web_frameworks', 
		{
